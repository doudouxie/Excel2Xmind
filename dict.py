...
l = [
    {'我的（模块）': {'券包（子模块1）': {'优惠券中心（子模块2）': {'领取优惠券（功能点）': {
        'cases': {'/*1*/测试2，是否能领取未开始的优惠券/*（前置条件）进入我的>领取优惠券*/': {'1. 选择还未开始的优惠券\n2. 点击领取': '1. 提示：未到领取时间'}}}}}}},
    {'我的（模块）': {'券包（子模块1）': {'优惠券中心（子模块2）': {'领取优惠券（功能点）': {
        'cases': {'/*6*/tc：测试1，是否能领取未开始的优惠券/*（前置条件）进入我的>领取优惠券*/': {'1. 选择还未开始的优惠券\n2. 点击领取': '1. 提示：未到领取时间'}}}}}}},
    {'X产品1（#105）': {'订单管理（子模块）': {'订单列表（子模块）': {'新增订单（功能点）': {'cases': {'/*1*/添加上架商品，选择有效用户，新增成功/*笔记：前置条件*/': {
        '1. 选择商品 c001\n2. 选择用户 nemo\n3. 选择收货地址\n4. 点击保存按钮': '1. 加载商品信息\n2. 加载用户信息\n3. 加载收货地址信息\n4. 提示保存成功'}}}}}}},
    {'X产品1（#105）': {'订单管理（子模块）': {'订单列表（子模块）': {'新增订单（功能点）': {'cases': {'/*2*/添加未上架商品，新增失败': {'Null': 'Null'}}}}}}},
    {'X产品1（#105）': {'订单管理（子模块）': {'订单列表（子模块）': {'新增订单（功能点）': {'cases': {'/*4*/添加已锁定用户，新增失败': {'Null': 'Null'}}}}}}}
]
